...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        // 这是一个同步块，使用 userAccount.intern() 返回的字符串对象作为同步锁。
        // 这意味着只有当多个线程尝试同时进入同步块，并且它们的 userAccount 字符串表示在常量池中是同一个对象时，
        // 才会被限制在同一时刻只有一个线程可以执行同步块内的代码。
        synchronized (userAccount.intern()) {
            ...
            // 3. 插入数据
            User user = new User();
            user.setUserName("用户" + IdUtil.simpleUUID());
